...
class DeterministicReversalLearningSession(
    ...
    def __init__(self, *args, **kwargs):
        ...
        # initialise previous total successes and failures
        self.prev_success_total = 0.0
        self.prev_failure_total = 0.0
        ...
        if subject_name:
            sessions = iterate_previous_sessions(
                ...
                n=1, # only need most recent session because it is cumulative
            )
            ...
            if sessions:
                ...
                # Load final success and failure total from the last trial of the previous day
                if "success_total" in prev_trials_table.columns and "failure_total" in prev_trials_table.columns:
                    self.prev_success_total = float(prev_trials_table["success_total"].iloc[-1])
                    self.prev_failure_total = float(prev_trials_table["failure_total"].iloc[-1])
                    log.warning(
                        f"Loaded previous success_total: {self.prev_success_total:.4f}, failure_total: {self.prev_failure_total:.4f}"
                    )
            else:
    ...
    def bayesian_strategy_analysis(self):
        if self.trial_num == 0:
            ...
            self.failure_total = self.prev_failure_total
...
class TrainingDeterministicReversalLearningSession(
    ...
    TrialDataModel = TrainingDeterministicReversalLearningTrialData
