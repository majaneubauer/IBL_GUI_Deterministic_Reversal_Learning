...
def get_boolean_pointer(camera: PySpin.CameraPtr, node_name: str) -> PySpin.CBooleanPtr:
    node = get_node(camera, node_name)
    pointer = PySpin.CBooleanPtr(node)
    assert pointer.IsValid(), f'Invalid CBooleanPtr {pointer.GetName()} for camera #{camera.DeviceID()}'
    return pointer
...
@process_camera
def enable_acquisition_frame_rate(enable: bool, camera: PySpin.CameraPtr) -> bool:
    try:
        enable_frame_rate_ptr = get_boolean_pointer(camera, 'AcquisitionFrameRateEnabled')
        if enable_frame_rate_ptr.GetValue() != enable:
            enable_frame_rate_ptr.SetValue(enable)
            camera_log(logging.INFO, camera, f'{"Enabled" if enable else "Disabled"} manual acquisition frame rate')
        return True
    except Exception as e:
        return camera_log(logging.ERROR, camera, f'Error setting frame rate: {e.args[0]}')
